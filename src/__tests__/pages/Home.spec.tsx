...
import App, { getStaticProps } from '../../pages/posts';
...
describe('Home', () => {
  ...
  it('should be able to navigate to post page after a click', () => {
    ...
    expect(mockedPush).toHaveBeenNthCalledWith(
      1,
      '/posts/como-utilizar-hooks',
      { forceOptimisticNavigation: false }
    );
    expect(mockedPush).toHaveBeenNthCalledWith(
      2,
      '/posts/criando-um-app-cra-do-zero',
      { forceOptimisticNavigation: false }
    );
  });
  ...
  it('should be able to load more posts if available', async () => {
    ...
    const { rerender } = render(<App postsPagination={postsPagination} />);
    ...
    mockedFetch.mockResolvedValueOnce({
      json: async () => Promise.resolve(newPost),
    });
    ...
    rerender(<App postsPagination={postsPagination} />);
